...
impl<T> ModuleNameTypeMap<T> {
  ...
  pub fn drain(
    ...
    for (ty, value) in self.map_index {
      for (key, value) in self.submaps.get_mut(value).unwrap().drain() {
        entries.push((ty.clone(), key, value));
      }
    }
    // HashMaps iterate in randomized per-process order; sort so the snapshot
    // sidecar is byte-identical across builds.
    entries.sort_by(|a, b| {
      let a_type = module_type_sort_key(&a.0);
      let b_type = module_type_sort_key(&b.0);
      (a_type, a.1.as_str()).cmp(&(b_type, b.1.as_str()))
    });
    for (index, (ty, key, value)) in entries.into_iter().enumerate() {
      f(index, &ty, key, value);
    }
  }
}
