...
impl MessagePort {
    pub fn send(&self, state: &mut OpState, data: JsMessageData) -> Result<(), MessagePortError> {
        let transferables = deserialize_js_transferables(state, data.transferables)?;
    ...
    pub async fn recv(
        ...
        if let Some((data, transferables)) = maybe_data {
            let js_transferables = serialize_transferables(&mut state.borrow_mut(), transferables);
...
impl Resource for MessagePortResource {
    ...
    fn transfer(self: Rc<Self>) -> Result<Box<dyn TransferredResource>, JsErrorBox> {
        self.cancel.cancel();
        let resource =
            Rc::try_unwrap(self).map_err(|_| JsErrorBox::from_err(MessagePortError::NotReady))?;
...
#[op2]
#[serde]
pub fn op_message_port_create_entangled(state: &mut OpState) -> (ResourceId, ResourceId) {
