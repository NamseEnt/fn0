...
pub fn op_compression_new(
    ...
    let inner = match (format, is_decoder) {
        ...
        ("deflate", false) => Inner::DeflateEncoder(ZlibEncoder::new(w, Compression::default())),
        ...
        ("gzip", false) => Inner::GzEncoder(GzEncoder::new(w, Compression::default())),
...
pub fn op_compression_finish(
    ...
    let out = match inner {
        Inner::DeflateDecoder(d) => d.finish().map_err(CompressionError::IoTypeError),
        Inner::DeflateEncoder(d) => d.finish().map_err(CompressionError::IoTypeError),
        Inner::DeflateRawDecoder(d) => d.finish().map_err(CompressionError::IoTypeError),
        Inner::DeflateRawEncoder(d) => d.finish().map_err(CompressionError::IoTypeError),
