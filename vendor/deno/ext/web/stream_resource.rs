...
impl BoundedBufferChannelInner {
    ...
    pub fn read(&mut self, limit: usize) -> Result<Option<BufView>, StreamResourceError> {
    ...
    pub fn write(&mut self, buffer: V8Slice<u8>) -> Result<(), V8Slice<u8>> {
        ...
        unsafe {
            *self.buffers.get_unchecked_mut(self.ring_producer as usize) = MaybeUninit::new(buffer)
        };
    ...
    pub fn can_read(&self) -> bool {
        ...
        self.closed || self.error.is_some() || self.ring_consumer != self.ring_producer
    }
...
impl BoundedBufferChannel {
    ...
    pub fn read(&self, limit: usize) -> Result<Option<BufView>, StreamResourceError> {
...
impl ReadableStreamResource {
    ...
    async fn read(self: Rc<Self>, limit: usize) -> Result<BufView, StreamResourceError> {
...
impl Resource for ReadableStreamResource {
    ...
    fn read(self: Rc<Self>, limit: usize) -> AsyncResult<BufView> {
        Box::pin(
            ReadableStreamResource::read(self, limit).map_err(deno_error::JsErrorBox::from_err),
        )
    }
    ...
    fn with_isolate<T>(mut f: impl FnMut(&mut v8::Isolate) -> T) -> T {
        ...
        ISOLATE.with(|cell| {
            let mut isolate = cell
                .get_or_init(|| std::sync::Mutex::new(v8::Isolate::new(Default::default())))
                .try_lock()
                .unwrap();
            f(&mut isolate)
        })
