...
impl Target {
    pub(crate) fn parse(val: &str) -> Option<Self> {
        ...
        if let Some(encoded_path) = val.strip_prefix("unix:") {
            ...
            let decoded =
                std::ffi::OsString::from_vec(percent_decode_str(encoded_path).collect::<Vec<u8>>());
    ...
    C::Error: Into<BoxError> + 'static,
{
    ...
    fn call(&mut self, orig_dst: Uri) -> Self::Future {
            ...
            return match intercept.target {
                ...
                Target::Tcp {
                    ...
                    let Ok(uri) = format!("http://{}:{}", host, port).parse() else {
                        return Box::pin(async { Err("failed to parse tcp proxy uri".into()) });
                    };
        ...
        let mut connector = HttpsConnector::from((self.http.clone(), self.tls.clone()));
        Box::pin(
            connector
                .call(orig_dst)
    ...
    T: hyper::rt::Read + hyper::rt::Write + Unpin,
{
    ...
    fn poll_write_vectored(
        ...
        match *self {
            Proxied::PassThrough(ref mut p) => Pin::new(p).poll_write_vectored(cx, bufs),
            Proxied::Tcp(ref mut p) => Pin::new(p).poll_write_vectored(cx, bufs),
            Proxied::HttpTunneled(ref mut p) => Pin::new(p).poll_write_vectored(cx, bufs),
...
fn test_no_proxy_ip_ranges() {
    let no_proxy =
        NoProxy::from_string(".foo.bar, bar.baz,10.42.1.1/24,::1,10.124.7.8,2001::/17").unwrap();
