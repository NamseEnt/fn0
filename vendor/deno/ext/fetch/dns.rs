...
pub type Resolving = Pin<Box<dyn Future<Output = Result<SocketAddrs, io::Error>> + Send>>;
        ...
        let task = match self {
            ...
            Resolver::Hickory(async_resolver) => {
                ...
                tokio::spawn(async move {
                    ...
                    let x: Vec<_> = result.into_iter().map(|x| SocketAddr::new(x, 0)).collect();
                    let iter: SocketAddrs = x.into_iter();
                    Ok(iter)
                })
            }
...
mod tests {
    ...
    async fn custom_dns_resolver() {
        let mut resolver =
            Resolver::Custom(Arc::new(DebugResolver("127.0.0.1:8080".parse().unwrap())));
