...
pub fn create_client_from_options(options: &Options) -> Result<Client, HttpClientCreateError> {
    create_http_client(
        &options.user_agent,
...
pub fn op_fetch(
    ...
    let (request_rid, cancel_handle_rid) = match scheme {
        "file" => {
            ...
            let (future, maybe_cancel_handle) = file_fetch_handler.fetch_file(state, &url);
            let request_rid = state
                .resource_table
                .add(FetchRequestResource { future, url });
            let maybe_cancel_handle_rid =
                maybe_cancel_handle.map(|ch| state.resource_table.add(FetchCancelHandle(ch)));
        ...
        "data" => {
            let data_url = DataUrl::process(url.as_str()).map_err(FetchError::DataUrl)?;
...
type CancelableResponseResult = Result<Result<http::Response<ResBody>, FetchError>, Canceled>;
...
impl Resource for FetchResponseResource {
    ...
    fn read(self: Rc<Self>, limit: usize) -> AsyncResult<BufView> {
        Box::pin(async move {
            ...
            fut.try_or_cancel(cancel_handle)
                .await
...
pub fn create_http_client(
    ...
    let mut tls_config = deno_tls::create_client_config(deno_tls::TlsClientConfigOptions {
    ...
    let mut http_connector = HttpConnector::new_with_resolver(options.dns_resolver.clone());
    ...
    let user_agent = user_agent
        .parse::<HeaderValue>()
        .map_err(|_| HttpClientCreateError::InvalidUserAgent(user_agent.to_string()))?;
...
pub struct Client {
    inner: Decompression<
        retry::Retry<FetchRetry, hyper_util::client::legacy::Client<Connector, ReqBody>>,
    >,
    connector: Connector,
...
impl Client {
    pub async fn connect(
        ...
        impl tokio::io::AsyncRead + tokio::io::AsyncWrite + Connection + Unpin + Send + 'static,
        ClientConnectError,
    > {
...
impl ReqBody {
    ...
    where
        B: hyper::body::Body<Data = Bytes, Error = JsErrorBox> + Send + Sync + 'static,
    {
...
impl hyper::body::Body for ReqBody {
    ...
    fn poll_frame(
        ...
        match &mut *self {
            ReqBody::Full(b) => Pin::new(b).poll_frame(cx).map_err(|never| match never {}),
            ReqBody::Empty(b) => Pin::new(b).poll_frame(cx).map_err(|never| match never {}),
...
impl<ResBody, E> retry::Policy<http::Request<ReqBody>, http::Response<ResBody>, E> for FetchRetry
where
