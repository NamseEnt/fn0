...
impl SimpleCache {
    ...
    async fn build_bundle(&self, env_vars: Vec<(String, String)>) -> Result<Arc<Bundle>, CacheError> {
        ...
        let (js, deployment) = if self.js_path.is_empty() {
            ...
            let js_str = String::from_utf8(js_bytes)
                .map_err(|e| CacheError::Decode(anyhow::anyhow!(e)))?;
