...
if remote[:2] == ["bash", "-c"]:
    command = remote[2]
    if "tar -xzf" in command and "manifest.json" in command:
        if os.environ.get("MOCK_SNAPSHOT_CHECKSUM_MISMATCH") == "1":
            record("snapshot-transfer-failed", "checksum mismatch")
            raise SystemExit(9)
        expected_sha, compressed = stdin_data.split(b"\n", 1)
        archive = gzip.decompress(compressed)
        with tarfile.open(fileobj=io.BytesIO(archive)) as snapshot_archive:
            manifest = snapshot_archive.extractfile("./manifest.json").read()
            if hashlib.sha256(manifest).hexdigest().encode("ascii") != expected_sha:
                raise SystemExit(9)
        record("snapshot-transfer", expected_sha.decode("ascii"))
        raise SystemExit(0)
...
mkdir -m 700 "${state_dir}/inventory-snapshot"
printf '{}' >"${state_dir}/inventory-snapshot/manifest.json"
inventory_output="$(bash "${mock_repo}/scripts/run-dodb-migration.sh" inventory --snapshot "${state_dir}/inventory-snapshot" --json --page-size 3)"
[[ "${inventory_output}" == '{"projects":[],"rows":0,"bytes":0}' ]]
[[ ! -e "${state_dir}/pulumi-outputs" ]]
...
assert data["args"][-6:] == ["inventory", "--snapshot", os.path.join(os.environ["MOCK_STATE"], "inventory-snapshot"), "--json", "--page-size", "3"]
assert data["token"] is None
assert data["suffix"] is None
PY

if bash "${mock_repo}/scripts/run-dodb-migration.sh" migrate --snapshot "${state_dir}/inventory-snapshot" >/dev/null 2>&1; then
  exit 1
fi
[[ ! -e "${state_dir}/oci-calls" ]]

bash "${mock_repo}/scripts/run-dodb-migration.sh" transport-check >/dev/null
[[ "$(cat "${state_dir}/pulumi-outputs")" == $'workerBastionId\nworkerSshPrivateKey\ndodbPrivateIp' ]]
...
mkdir -m 700 "${state_dir}/source-snapshot"
printf '{}' >"${state_dir}/source-snapshot/manifest.json"
printf test >"${state_dir}/source-snapshot/db.sqlite"
bash "${mock_repo}/scripts/run-dodb-migration.sh" verify --snapshot "${state_dir}/source-snapshot" --project-id 00000001 --json >/dev/null
python3 - "${state_dir}" <<'PY'
import json
import os
import sys
...
assert arguments[1] == "--snapshot"
assert arguments[2].startswith("/tmp/fn0-db-migrate.") and arguments[2].endswith("/snapshot")
assert arguments[3:] == ["--project-id", "00000001", "--json"]
env = open(os.path.join(state_dir, "remote-env"), encoding="utf-8").read()
assert env == ""
assert os.path.exists(os.path.join(state_dir, "snapshot-transfer"))
...
PY
remote_run_count="$(wc -l <"${state_dir}/remote-runs" | tr -d '[:space:]')"
if MOCK_SNAPSHOT_CHECKSUM_MISMATCH=1 bash "${mock_repo}/scripts/run-dodb-migration.sh" verify --snapshot "${state_dir}/source-snapshot" --project-id 00000001 --json >/dev/null 2>&1; then
  exit 1
fi
[[ "$(wc -l <"${state_dir}/remote-runs" | tr -d '[:space:]')" == "${remote_run_count}" ]]
rg -q 'checksum mismatch' "${state_dir}/snapshot-transfer-failed"
