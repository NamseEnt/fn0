...
impl DocDb {
    ...
    pub async fn current_build(&self, subdomain: &str) -> Result<Option<String>> {
        let conn = self.db.connect()?;
        let pk = format!("forte_builds:{subdomain}");
        let row = conn
            .query(
                "SELECT value FROM docs WHERE pk = ? ORDER BY sk DESC LIMIT 1",
                libsql::params!(pk),
            )
            .await?
            .next()
    ...
    pub async fn enqueue_r2_prefix_delete(
        &self,
        subdomain: &str,
        build_id: &str,
    ) -> Result<()> {
...
mod tests {
    ...
    async fn register_build_returns_previous_ids() {
        ...
        let current = db.current_build("alice-app").await.unwrap();
        assert_eq!(current.as_deref(), Some("build-b"));
    }
