...
pub async fn handle_deploy_start(
    ...
    let code_version = match ctx.doc_db.next_code_version(project.code_id).await {
        Ok(v) => v,
        Err(e) => {
            return json_response(
                500,
    ...
    let s3_key = format!("sources/{}/{code_version}.raw.tar", project.subdomain);
    ...
    let presigning_config = match PresigningConfig::expires_in(std::time::Duration::from_secs(300))
    {
        ...
        Err(_) => {
            return json_response(
                500,
                &ErrorResponse {
                    error: "Failed to create presigning config".to_string(),
                },
            );
        }
    };
...
pub async fn handle_deploy_finish(
    ...
    if let Err(e) = ctx
        .doc_db
        .insert_deployment(&request.subdomain, request.code_id, request.code_version)
        .await
    {
        return json_response(
            500,
    ...
    json_response(
        200,
        &serde_json::json!({"ok": true, "code_version": request.code_version}),
    )
}
