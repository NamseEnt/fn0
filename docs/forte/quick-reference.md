...
forte purge-page /path /path # invalidate CDN edge copy of cache_static pages (runtime data changed)
...
| Singleton WebSocket | `rs/src/ws_singleton/` | one current assignment per project; temporary overlap is possible during lease expiry | `connect` returns options, `on_message`, optional `on_connect`/`on_disconnect` |
...
use forte_sdk::websocket::{ConnectDecision, ConnectEvent, MessageEvent, WebSocketMessage};
...
## Per-page Head (`<title>` and meta tags)
...
export const head: HeadDescriptor[] = [
    { title: "My App" },
    { name: "description", content: "The default app description." },
];
...
Per-page override in the page module (`head` function receives the same `Props` as the component):
...
The page `head` merges over the app defaults — same `name`/`property` keys replace the app entry, others are kept. See [head.md](head.md) for full merge semantics and descriptor shapes.
