...
> **Scaffold note:** `forte add action` generates starter code with wrong names (`{Name}Input`, `{Name}Output`, `pub async fn action`). This code will not be discovered by codegen. Rename the structs to `Input` / `Output` and the function to `handler` before running `forte build`.

## TypeScript Client

`forte-rs-to-ts` auto-generates a typed client for each action in `fe/src/actions/.generated/<name>.ts`. Do not call the action URL manually — import from the generated file instead:
...
const InputSchema = z.object({ email: z.string(), password: z.string() });
...
Import and call from your frontend code:

```ts
import { userLogin } from "../actions/.generated/user_login";
// or via the barrel: import { userLogin } from "../actions/.generated";

const result = await userLogin({ email, password });
if (result.t === "Ok") { /* result.token */ }
```

> **Scaffold note:** The TypeScript file generated by `forte add action` is a manual `fetch` wrapper that uses the wrong route prefix (`/_action/...` instead of `/__forte_action/...`) and imports from a non-existent `.types` file. Discard it — the correct client is generated automatically from the Rust types.
