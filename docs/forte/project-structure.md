...
```
<name>/
├── Forte.toml                 # Project config (name, project_id after first deploy)
├── Cargo.toml                 # Workspace root (members = ["rs"])
├── .gitignore
├── cron.yaml                  # Optional: scheduled queue tasks
├── env.yaml                   # Optional: encrypted environment variables (bundled on deploy)
│
...
│       ├── hooks/             # Self-invoke hooks (no mod.rs; wired into route_generated.rs)
...
Created by `forte init`. After the first `forte deploy`, the assigned `project_id` is written back automatically:

```toml
[project]
name = "my-app"
project_id = "abc123"   # written by forte deploy on first deployment
```

`project_id` is the stable identifier used for all subsequent deployments. Do not edit it manually.
