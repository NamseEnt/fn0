...
dodb_control_db_call() {
  dodb_control_db_open
  local command_name="$1" remote_command argument quoted_argument
  shift
  local -a remote_arguments=(
    "$DODB_CONTROL_DB_REMOTE_BINARY"
    --dodb-root-cert "$DODB_CONTROL_DB_REMOTE_CERT_PATH"
    "$command_name"
    "$@"
  )
  ...
  ssh "${DODB_CONTROL_DB_SSH_OPTIONS[@]}" opc@127.0.0.1 "$remote_command"
}
