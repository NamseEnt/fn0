...
usage() {
  cat <<'USAGE'
Usage:
  scripts/run-dodb-migration.sh snapshot --output <secure-dir>
  scripts/run-dodb-migration.sh inventory --snapshot <secure-dir> [migration options]
  scripts/run-dodb-migration.sh transport-check
  scripts/run-dodb-migration.sh verify --snapshot <secure-dir> [migration options]
  scripts/run-dodb-migration.sh migrate --snapshot <secure-dir> --apply [migration options]

snapshot is the only command that calls Turso. inventory reads the local SQLite snapshot. migrate and verify transfer and read only the snapshot on the dodb VM. The migrate command requires --apply.
USAGE
}
...
if [[ "$#" -eq 2 && "${2}" == "--help" ]]; then
  case "${1}" in snapshot|inventory|transport-check|verify|migrate) usage; exit 0 ;; esac
fi
if [[ "$#" -lt 1 ]]; then
  usage >&2
  exit 2
...
case "${command_name}" in
  snapshot|inventory|transport-check|verify|migrate) ;;
...
if [[ "${command_name}" == "snapshot" ]]; then
  exec python3 "${REPO_ROOT}/scripts/create-dodb-snapshot.py" "$@"
fi
...
previous_argument=""
for argument in "$@"; do
  if [[ "${previous_argument}" == "--snapshot" ]]; then snapshot_dir="${argument}"; fi
  previous_argument="${argument}"
done
if [[ "${command_name}" == "migrate" || "${command_name}" == "verify" ]]; then
  if [[ -z "${snapshot_dir}" || ! -d "${snapshot_dir}" || ! -f "${snapshot_dir}/manifest.json" ]]; then
    echo "migrate and verify require --snapshot pointing to a directory with manifest.json" >&2
    exit 2
  fi
  cargo run --quiet --locked -p fn0-db-migrate -- inventory --snapshot "${snapshot_dir}" --json >/dev/null
fi
...
cleanup() {
  if [[ "${remote_cleanup_needed}" == "true" && -n "${BASTION_TUNNEL_PID:-}" ]]; then
...
remote_snapshot_dir="${remote_temp_dir}/snapshot"
...
: >"${env_file}"
chmod 600 "${env_file}"
REMOTE_SNAPSHOT_DIR="${remote_snapshot_dir}" python3 - "${argument_file}" "${command_name}" "$@" <<'PY'
import json
import os
import sys
...
with open(sys.argv[1], "w", encoding="utf-8") as argument_file:
    ...
    for argument_index, argument in enumerate(arguments[:-1]):
        if argument == "--snapshot":
            arguments[argument_index + 1] = os.environ["REMOTE_SNAPSHOT_DIR"]
...
if [[ "${command_name}" == "migrate" || "${command_name}" == "verify" ]]; then
  manifest_sha="$(sha256sum "${snapshot_dir}/manifest.json" | awk '{print $1}')"
  {
    printf '%s\n' "${manifest_sha}"
    tar -czf - -C "${snapshot_dir}" .
  } | ssh "${ssh_options[@]}" opc@127.0.0.1 "bash -c 'set -euo pipefail; IFS= read -r expected_sha; [[ \"\$expected_sha\" =~ ^[0-9a-f]{64}\$ ]]; snapshot_dir=\"\$1\"; [[ \"\$snapshot_dir\" =~ ^/tmp/fn0-db-migrate\\.[0-9a-f]{24}/snapshot\$ ]]; umask 077; mkdir -m 700 -p \"\$snapshot_dir\"; tar -xzf - -C \"\$snapshot_dir\"; actual_sha=\"\$(sha256sum \"\$snapshot_dir/manifest.json\" | awk '\''{print \$1}'\'')\"; [[ \"\$actual_sha\" == \"\$expected_sha\" ]]; printf '\''snapshot manifest checksum verified: %s\\n'\'' \"\$actual_sha\"' _ '${remote_snapshot_dir}'"
  echo ">> Transferred and verified snapshot over SSH stdin" >&2
fi
...
umask 077
mkdir -m 700 -p "$remote_temp_dir"
chmod 700 "$remote_temp_dir"
