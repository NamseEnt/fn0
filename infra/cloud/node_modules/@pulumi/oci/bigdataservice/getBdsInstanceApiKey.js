"use strict";
// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***
Object.defineProperty(exports, "__esModule", { value: true });
exports.getBdsInstanceApiKeyOutput = exports.getBdsInstanceApiKey = void 0;
const pulumi = require("@pulumi/pulumi");
const utilities = require("../utilities");
/**
 * This data source provides details about a specific Bds Instance Api Key resource in Oracle Cloud Infrastructure Big Data Service service.
 *
 * Returns the user's API key information for the given ID.
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as oci from "@pulumi/oci";
 *
 * const testBdsInstanceApiKey = oci.BigDataService.getBdsInstanceApiKey({
 *     apiKeyId: testApiKey.id,
 *     bdsInstanceId: testBdsInstance.id,
 * });
 * ```
 */
function getBdsInstanceApiKey(args, opts) {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("oci:BigDataService/getBdsInstanceApiKey:getBdsInstanceApiKey", {
        "apiKeyId": args.apiKeyId,
        "bdsInstanceId": args.bdsInstanceId,
    }, opts);
}
exports.getBdsInstanceApiKey = getBdsInstanceApiKey;
/**
 * This data source provides details about a specific Bds Instance Api Key resource in Oracle Cloud Infrastructure Big Data Service service.
 *
 * Returns the user's API key information for the given ID.
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as oci from "@pulumi/oci";
 *
 * const testBdsInstanceApiKey = oci.BigDataService.getBdsInstanceApiKey({
 *     apiKeyId: testApiKey.id,
 *     bdsInstanceId: testBdsInstance.id,
 * });
 * ```
 */
function getBdsInstanceApiKeyOutput(args, opts) {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("oci:BigDataService/getBdsInstanceApiKey:getBdsInstanceApiKey", {
        "apiKeyId": args.apiKeyId,
        "bdsInstanceId": args.bdsInstanceId,
    }, opts);
}
exports.getBdsInstanceApiKeyOutput = getBdsInstanceApiKeyOutput;
//# sourceMappingURL=getBdsInstanceApiKey.js.map