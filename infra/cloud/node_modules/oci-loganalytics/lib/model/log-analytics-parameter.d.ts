/**
 * LogAnalytics API
 * The LogAnalytics API for the LogAnalytics service.

 * OpenAPI spec version: 20200601
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2026, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */
/**
 * LogAnalyticsParameter
 */
export interface LogAnalyticsParameter {
    /**
     * The default value of the parameter.
     */
    "defaultValue"?: string;
    /**
     * The parameter description.
     */
    "description"?: string;
    /**
     * A flag indicating whether or not the parameter is active.
     *
     */
    "isActive"?: boolean;
    /**
     * The parameter name.
     */
    "name"?: string;
    /**
     * The source unique identifier. Note: Numbers greater than Number.MAX_SAFE_INTEGER will result in rounding issues.
     */
    "sourceId"?: number;
}
export declare namespace LogAnalyticsParameter {
    function getJsonObj(obj: LogAnalyticsParameter): object;
    function getDeserializedJsonObj(obj: LogAnalyticsParameter): object;
}
