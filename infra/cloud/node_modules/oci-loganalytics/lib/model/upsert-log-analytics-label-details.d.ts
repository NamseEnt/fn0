/**
 * LogAnalytics API
 * The LogAnalytics API for the LogAnalytics service.

 * OpenAPI spec version: 20200601
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2026, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */
import * as model from "../model";
/**
 * Upsert LogAnalytics Label Details
 */
export interface UpsertLogAnalyticsLabelDetails {
    /**
     * The alias list.
     */
    "aliases"?: Array<model.LogAnalyticsLabelAlias>;
    /**
     * suggest type Note: Numbers greater than Number.MAX_SAFE_INTEGER will result in rounding issues.
     */
    "suggestType"?: number;
    /**
     * The label description.
     */
    "description"?: string;
    /**
     * The label display name.
     */
    "displayName"?: string;
    /**
     * The edit version. Note: Numbers greater than Number.MAX_SAFE_INTEGER will result in rounding issues.
     */
    "editVersion"?: number;
    /**
     * The label impact.
     */
    "impact"?: string;
    /**
     * The system flag.  A value of false denotes a custom, or user
     * defined label.  A value of true denotes a built in label.
     *
     */
    "isSystem"?: boolean;
    /**
     * The label name.
     */
    "name"?: string;
    /**
     * The label priority. Valid values are (NONE, LOW, HIGH). NONE is default.
     */
    "priority"?: UpsertLogAnalyticsLabelDetails.Priority;
    /**
     * The label recommendation.
     */
    "recommendation"?: string;
    /**
     * The label type. Valid values are (INFO, PROBLEM). INFO is default.
     */
    "type"?: UpsertLogAnalyticsLabelDetails.Type;
}
export declare namespace UpsertLogAnalyticsLabelDetails {
    enum Priority {
        None = "NONE",
        Low = "LOW",
        Medium = "MEDIUM",
        High = "HIGH"
    }
    enum Type {
        Info = "INFO",
        Problem = "PROBLEM"
    }
    function getJsonObj(obj: UpsertLogAnalyticsLabelDetails): object;
    function getDeserializedJsonObj(obj: UpsertLogAnalyticsLabelDetails): object;
}
