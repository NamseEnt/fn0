/**
 * LogAnalytics API
 * The LogAnalytics API for the LogAnalytics service.

 * OpenAPI spec version: 20200601
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2026, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */
import * as model from "../model";
/**
 * LogAnalyticsParserFilter
 */
export interface LogAnalyticsParserFilter {
    /**
     * The parser filter unique identifier.
     */
    "id"?: string;
    "parser"?: model.LogAnalyticsParser;
    /**
     * The agent version.
     */
    "agentVersion"?: string;
    /**
     * A flag indicating whether or not the filter is currently being used.
     *  Note: Numbers greater than Number.MAX_SAFE_INTEGER will result in rounding issues.
     */
    "isInUse"?: number;
    /**
     * The operating system.
     */
    "operatingSystem"?: string;
    /**
     * The parser unique identifier. Note: Numbers greater than Number.MAX_SAFE_INTEGER will result in rounding issues.
     */
    "parserId"?: number;
    /**
     * The version.
     */
    "version"?: string;
}
export declare namespace LogAnalyticsParserFilter {
    function getJsonObj(obj: LogAnalyticsParserFilter): object;
    function getDeserializedJsonObj(obj: LogAnalyticsParserFilter): object;
}
