/**
 * MarketplacePublisherService API
 * Use the Marketplace Publisher API to manage the publishing of applications in Oracle Cloud Infrastructure Marketplace.
 * OpenAPI spec version: 20241201
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2026, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */
/**
 * The model for the supportDoc details.
 */
export interface SupportDoc {
    /**
     * The name for the supportDoc.
     */
    "name": string;
    /**
     * The code for the supportDoc.
     */
    "code": string;
    /**
     * The supportDoc group for the supportDoc.
     */
    "supportDocGroup": string;
    /**
     * The description of the supportDoc.
     */
    "description"?: string;
    /**
     * The URL of the specified attachment.
     */
    "contentUrl"?: string;
    /**
     * The MIME type of the attachment.
     */
    "mimeType"?: string;
    /**
     * The file name of the attachment.
     */
    "fileName"?: string;
    /**
     * The current state for the supportDoc.
     */
    "lifecycleState": SupportDoc.LifecycleState;
    /**
      * The date and time the supportDoc was created, in the format defined by [RFC3339](https://tools.ietf.org/html/rfc3339).
  * <p>
  Example: {@code 2022-09-15T21:10:29.600Z}
  *
      */
    "timeCreated": Date;
    /**
      * The date and time the supportDoc was updated, in the format defined by [RFC3339](https://tools.ietf.org/html/rfc3339).
  * <p>
  Example: {@code 2022-09-15T21:10:29.600Z}
  *
      */
    "timeUpdated": Date;
}
export declare namespace SupportDoc {
    enum LifecycleState {
        Active = "ACTIVE",
        Inactive = "INACTIVE",
        /**
         * This value is used if a service returns a value for this enum that is not recognized by this
         * version of the SDK.
         */
        UnknownValue = "UNKNOWN_VALUE"
    }
    function getJsonObj(obj: SupportDoc): object;
    function getDeserializedJsonObj(obj: SupportDoc): object;
}
