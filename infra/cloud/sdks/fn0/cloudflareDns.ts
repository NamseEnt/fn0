// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "./utilities";

export class CloudflareDns extends pulumi.ComponentResource {
    /** @internal */
    public static readonly __pulumiType = 'fn0:index:CloudflareDns';

    /**
     * Returns true if the given object is an instance of CloudflareDns.  This is designed to work even
     * when multiple copies of the Pulumi SDK have been loaded into the same process.
     */
    public static isInstance(obj: any): obj is CloudflareDns {
        if (obj === undefined || obj === null) {
            return false;
        }
        return obj['__pulumiType'] === CloudflareDns.__pulumiType;
    }

    declare public /*out*/ readonly certificate: pulumi.Output<string>;
    declare public /*out*/ readonly dnsApiToken: pulumi.Output<string>;
    declare public /*out*/ readonly privateKeyPem: pulumi.Output<string>;
    declare public /*out*/ readonly saasApiToken: pulumi.Output<string>;

    /**
     * Create a CloudflareDns resource with the given unique name, arguments, and options.
     *
     * @param name The _unique_ name of the resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param opts A bag of options that control this resource's behavior.
     */
    constructor(name: string, args: CloudflareDnsArgs, opts?: pulumi.ComponentResourceOptions) {
        let resourceInputs: pulumi.Inputs = {};
        opts = opts || {};
        if (!opts.id) {
            if (args?.accountId === undefined && !opts.urn) {
                throw new Error("Missing required property 'accountId'");
            }
            if (args?.domain === undefined && !opts.urn) {
                throw new Error("Missing required property 'domain'");
            }
            if (args?.suffix === undefined && !opts.urn) {
                throw new Error("Missing required property 'suffix'");
            }
            if (args?.zoneId === undefined && !opts.urn) {
                throw new Error("Missing required property 'zoneId'");
            }
            resourceInputs["accountId"] = args?.accountId;
            resourceInputs["domain"] = args?.domain;
            resourceInputs["suffix"] = args?.suffix;
            resourceInputs["zoneId"] = args?.zoneId;
            resourceInputs["certificate"] = undefined /*out*/;
            resourceInputs["dnsApiToken"] = undefined /*out*/;
            resourceInputs["privateKeyPem"] = undefined /*out*/;
            resourceInputs["saasApiToken"] = undefined /*out*/;
        } else {
            resourceInputs["certificate"] = undefined /*out*/;
            resourceInputs["dnsApiToken"] = undefined /*out*/;
            resourceInputs["privateKeyPem"] = undefined /*out*/;
            resourceInputs["saasApiToken"] = undefined /*out*/;
        }
        opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts);
        super(CloudflareDns.__pulumiType, name, resourceInputs, opts, true /*remote*/);
    }
}

/**
 * The set of arguments for constructing a CloudflareDns resource.
 */
export interface CloudflareDnsArgs {
    accountId: pulumi.Input<string>;
    domain: pulumi.Input<string>;
    suffix: pulumi.Input<string>;
    zoneId: pulumi.Input<string>;
}
