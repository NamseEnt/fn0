/**
 * LogAnalytics API
 * The LogAnalytics API for the LogAnalytics service.

 * OpenAPI spec version: 20200601
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2026, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */
import * as model from "../model";
/**
 * Command descriptor for querylanguage MULTISEARCH command.
 *
 */
export interface MultiSearchCommandDescriptor extends model.AbstractCommandDescriptor {
    /**
     * List of sub-searches specified in query string as multisearch command arguments.
     *
     */
    "subQueries"?: Array<model.ParseQueryOutput>;
    "name": string;
}
export declare namespace MultiSearchCommandDescriptor {
    function getJsonObj(obj: MultiSearchCommandDescriptor, isParentJsonObj?: boolean): object;
    const name = "MULTI_SEARCH";
    function getDeserializedJsonObj(obj: MultiSearchCommandDescriptor, isParentJsonObj?: boolean): object;
}
