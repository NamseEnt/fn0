 ...
 * <p>
 **Notes**
 * - To specify OCPU core count, you must use either {@code ocpuCount} or {@code cpuCoreCount}. You cannot use both parameters at the same time. For Autonomous AI Database Serverless instances, {@code ocpuCount} is not used.
...
export interface UpdateAutonomousDatabaseDetails {
  ...
  *
     ...
     * {@code TRUE} if the Autonomous AI Database has Data Guard and Access Control enabled, and the Autonomous AI Database uses the primary's IP access control list (ACL) for standby.
     * {@code FALSE} if the Autonomous AI Database has Data Guard and Access Control enabled, and the Autonomous AI Database uses a different IP access control list (ACL) for standby compared to primary.
     *
     */
  ...
  *
    ...
    /**
     * Indicates whether auto scaling is enabled for the Autonomous AI Database CPU core count. Setting to {@code TRUE} enables auto scaling. Setting to {@code FALSE} disables auto scaling. The default value is {@code TRUE}. Auto scaling is only available for [Autonomous AI Database Serverless instances](https://docs.oracle.com/en/cloud/paas/autonomous-database/serverless/).
     *
     */
  ...
  * <p>
  To create a local standby, set to {@code TRUE}. To delete a local standby, set to {@code FALSE}. For more information on using Autonomous Data Guard on an Autonomous AI Database Serverless instance (local and cross-region) , see [About Standby Databases](https://docs.oracle.com/en/cloud/paas/autonomous-database/adbsa/autonomous-data-guard-about.html#GUID-045AD017-8120-4BDC-AF58-7430FFE28D2B)
  * <p>
  ...
  *
    ...
    /**
     * The [OCID](https://docs.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of a dedicated resource pool leader Autonomous AI Database in the same region, that is required when local Autonomous Data Guard is enabled for a dedicated resource pool member using the parameter {@code isLocalDataGuardEnabled}.
...
export declare namespace UpdateAutonomousDatabaseDetails {
    enum ComputeModel {
        Ecpu = "ECPU",
        Ocpu = "OCPU"
    }
    ...
    function getJsonObj(obj: UpdateAutonomousDatabaseDetails): object;
    function getDeserializedJsonObj(obj: UpdateAutonomousDatabaseDetails): object;
}
