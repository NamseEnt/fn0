"use strict";
// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***
Object.defineProperty(exports, "__esModule", { value: true });
exports.getReplicationScheduleOutput = exports.getReplicationSchedule = void 0;
const pulumi = require("@pulumi/pulumi");
const utilities = require("../utilities");
/**
 * This data source provides details about a specific Replication Schedule resource in Oracle Cloud Infrastructure Cloud Migrations service.
 *
 * Gets a replication schedule by identifier.
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as oci from "@pulumi/oci";
 *
 * const testReplicationSchedule = oci.CloudMigrations.getReplicationSchedule({
 *     replicationScheduleId: testReplicationScheduleOciCloudMigrationsReplicationSchedule.id,
 * });
 * ```
 */
function getReplicationSchedule(args, opts) {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("oci:CloudMigrations/getReplicationSchedule:getReplicationSchedule", {
        "replicationScheduleId": args.replicationScheduleId,
    }, opts);
}
exports.getReplicationSchedule = getReplicationSchedule;
/**
 * This data source provides details about a specific Replication Schedule resource in Oracle Cloud Infrastructure Cloud Migrations service.
 *
 * Gets a replication schedule by identifier.
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as oci from "@pulumi/oci";
 *
 * const testReplicationSchedule = oci.CloudMigrations.getReplicationSchedule({
 *     replicationScheduleId: testReplicationScheduleOciCloudMigrationsReplicationSchedule.id,
 * });
 * ```
 */
function getReplicationScheduleOutput(args, opts) {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("oci:CloudMigrations/getReplicationSchedule:getReplicationSchedule", {
        "replicationScheduleId": args.replicationScheduleId,
    }, opts);
}
exports.getReplicationScheduleOutput = getReplicationScheduleOutput;
//# sourceMappingURL=getReplicationSchedule.js.map