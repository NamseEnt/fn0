import * as pulumi from "@pulumi/pulumi";
/**
 * This data source provides details about a specific Access Request Audit Log Report resource in Oracle Cloud Infrastructure Operator Access Control service.
 *
 * Gets the Audit Log Report for the given access requestId.
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as oci from "@pulumi/oci";
 *
 * const testAccessRequestAuditLogReport = oci.OperatorAccessControl.getAccessRequestAuditLogReport({
 *     accessRequestId: testAccessRequest.id,
 *     enableProcessTree: accessRequestAuditLogReportEnableProcessTree,
 * });
 * ```
 */
export declare function getAccessRequestAuditLogReport(args: GetAccessRequestAuditLogReportArgs, opts?: pulumi.InvokeOptions): Promise<GetAccessRequestAuditLogReportResult>;
/**
 * A collection of arguments for invoking getAccessRequestAuditLogReport.
 */
export interface GetAccessRequestAuditLogReportArgs {
    /**
     * unique AccessRequest identifier
     */
    accessRequestId: string;
    /**
     * To enable process tree computation in audit report
     */
    enableProcessTree?: number;
}
/**
 * A collection of values returned by getAccessRequestAuditLogReport.
 */
export interface GetAccessRequestAuditLogReportResult {
    readonly accessRequestId: string;
    /**
     * auditReportStatus for the accessRequestId
     */
    readonly auditReportStatus: string;
    readonly enableProcessTree?: number;
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
    /**
     * Contains the process tree data
     */
    readonly processTree: string;
    /**
     * Contains the report data.
     */
    readonly report: string;
    /**
     * Time when the audit report was generated [RFC 3339](https://tools.ietf.org/html/rfc3339) timestamp format.Example: '2020-05-22T21:10:29.600Z'
     */
    readonly timeOfReportGeneration: string;
}
/**
 * This data source provides details about a specific Access Request Audit Log Report resource in Oracle Cloud Infrastructure Operator Access Control service.
 *
 * Gets the Audit Log Report for the given access requestId.
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as oci from "@pulumi/oci";
 *
 * const testAccessRequestAuditLogReport = oci.OperatorAccessControl.getAccessRequestAuditLogReport({
 *     accessRequestId: testAccessRequest.id,
 *     enableProcessTree: accessRequestAuditLogReportEnableProcessTree,
 * });
 * ```
 */
export declare function getAccessRequestAuditLogReportOutput(args: GetAccessRequestAuditLogReportOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetAccessRequestAuditLogReportResult>;
/**
 * A collection of arguments for invoking getAccessRequestAuditLogReport.
 */
export interface GetAccessRequestAuditLogReportOutputArgs {
    /**
     * unique AccessRequest identifier
     */
    accessRequestId: pulumi.Input<string>;
    /**
     * To enable process tree computation in audit report
     */
    enableProcessTree?: pulumi.Input<number>;
}
