"use strict";
// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***
Object.defineProperty(exports, "__esModule", { value: true });
exports.getUserAssessmentOutput = exports.getUserAssessment = void 0;
const pulumi = require("@pulumi/pulumi");
const utilities = require("../utilities");
/**
 * This data source provides details about a specific User Assessment resource in Oracle Cloud Infrastructure Data Safe service.
 *
 * Gets a user assessment by identifier.
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as oci from "@pulumi/oci";
 *
 * const testUserAssessment = oci.DataSafe.getUserAssessment({
 *     userAssessmentId: testUserAssessmentOciDataSafeUserAssessment.id,
 * });
 * ```
 */
function getUserAssessment(args, opts) {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("oci:DataSafe/getUserAssessment:getUserAssessment", {
        "userAssessmentId": args.userAssessmentId,
    }, opts);
}
exports.getUserAssessment = getUserAssessment;
/**
 * This data source provides details about a specific User Assessment resource in Oracle Cloud Infrastructure Data Safe service.
 *
 * Gets a user assessment by identifier.
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as oci from "@pulumi/oci";
 *
 * const testUserAssessment = oci.DataSafe.getUserAssessment({
 *     userAssessmentId: testUserAssessmentOciDataSafeUserAssessment.id,
 * });
 * ```
 */
function getUserAssessmentOutput(args, opts) {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("oci:DataSafe/getUserAssessment:getUserAssessment", {
        "userAssessmentId": args.userAssessmentId,
    }, opts);
}
exports.getUserAssessmentOutput = getUserAssessmentOutput;
//# sourceMappingURL=getUserAssessment.js.map