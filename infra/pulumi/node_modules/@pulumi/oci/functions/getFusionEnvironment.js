"use strict";
// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***
Object.defineProperty(exports, "__esModule", { value: true });
exports.getFusionEnvironmentOutput = exports.getFusionEnvironment = void 0;
const pulumi = require("@pulumi/pulumi");
const utilities = require("../utilities");
/**
 * This data source provides details about a specific Fusion Environment resource in Oracle Cloud Infrastructure Fusion Apps service.
 *
 * Gets a FusionEnvironment by identifier
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as oci from "@pulumi/oci";
 *
 * const testFusionEnvironment = oci.Functions.getFusionEnvironment({
 *     fusionEnvironmentId: testFusionEnvironmentOciFusionAppsFusionEnvironment.id,
 * });
 * ```
 */
function getFusionEnvironment(args, opts) {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("oci:Functions/getFusionEnvironment:getFusionEnvironment", {
        "fusionEnvironmentId": args.fusionEnvironmentId,
    }, opts);
}
exports.getFusionEnvironment = getFusionEnvironment;
/**
 * This data source provides details about a specific Fusion Environment resource in Oracle Cloud Infrastructure Fusion Apps service.
 *
 * Gets a FusionEnvironment by identifier
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as oci from "@pulumi/oci";
 *
 * const testFusionEnvironment = oci.Functions.getFusionEnvironment({
 *     fusionEnvironmentId: testFusionEnvironmentOciFusionAppsFusionEnvironment.id,
 * });
 * ```
 */
function getFusionEnvironmentOutput(args, opts) {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("oci:Functions/getFusionEnvironment:getFusionEnvironment", {
        "fusionEnvironmentId": args.fusionEnvironmentId,
    }, opts);
}
exports.getFusionEnvironmentOutput = getFusionEnvironmentOutput;
//# sourceMappingURL=getFusionEnvironment.js.map