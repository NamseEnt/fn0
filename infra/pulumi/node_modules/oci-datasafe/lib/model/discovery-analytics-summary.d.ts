/**
 * Data Safe API
 * APIs for using Oracle Data Safe.
 * OpenAPI spec version: 20181201
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2026, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */
import * as model from "../model";
/**
 * Summary of discovery analytics data.
 */
export interface DiscoveryAnalyticsSummary {
    /**
     * The name of the aggregation metric.
     */
    "metricName": DiscoveryAnalyticsSummary.MetricName;
    "dimensions"?: model.Dimensions;
    /**
     * The total count for the aggregation metric. Note: Numbers greater than Number.MAX_SAFE_INTEGER will result in rounding issues.
     */
    "count": number;
    /**
     * The date and time when data discovery was last done on the target database, in the format defined by [RFC3339](https://tools.ietf.org/html/rfc3339).
     */
    "timeLastDiscovered"?: Date;
}
export declare namespace DiscoveryAnalyticsSummary {
    enum MetricName {
        SensitiveDataModel = "SENSITIVE_DATA_MODEL",
        SensitiveType = "SENSITIVE_TYPE",
        SensitiveSchema = "SENSITIVE_SCHEMA",
        SensitiveTable = "SENSITIVE_TABLE",
        SensitiveColumn = "SENSITIVE_COLUMN",
        SensitiveDataValue = "SENSITIVE_DATA_VALUE",
        TargetDatabase = "TARGET_DATABASE",
        /**
         * This value is used if a service returns a value for this enum that is not recognized by this
         * version of the SDK.
         */
        UnknownValue = "UNKNOWN_VALUE"
    }
    function getJsonObj(obj: DiscoveryAnalyticsSummary): object;
    function getDeserializedJsonObj(obj: DiscoveryAnalyticsSummary): object;
}
