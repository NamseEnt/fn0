...
function createDockerRegistry(parent, { compartmentId, suffix, region, }) {
    ...
    const user = new oci.identity.User("hq-user", {
        name: pulumi.interpolate `hq-user-${suffix}`,
        description: "User for HQ deployment",
    }, { parent });
    const dockerGroup = new oci.identity.Group("hq-docker-pusher-group", {
        name: pulumi.interpolate `hq-docker-pushers-${suffix}`,
        description: "Group allowed to push to OCIR",
    }, { parent });
    new oci.identity.UserGroupMembership("hq-membership", {
        userId: user.id,
        groupId: dockerGroup.id,
    }, { parent });
    new oci.identity.Policy("ocir-push-policy", {
        compartmentId,
    ...
    const authToken = new oci.identity.AuthToken("hq-auth-token", {
        userId: user.id,
