...
exports.AwsLambdaCwasmCompiler = void 0;
const pulumi = require("@pulumi/pulumi");
const aws = require("@pulumi/aws");
const path = require("node:path");
class AwsLambdaCwasmCompiler extends pulumi.ComponentResource {
    constructor(name, args, opts) {
        ...
        const wasmtimeLayer = new aws.lambda.LayerVersion("wasmtime-layer", {
            ...
            code: new pulumi.asset.FileArchive(path.join(__dirname, "./wasmtime-layer")),
        }, { parent: this });
        const zstdLayer = new aws.lambda.LayerVersion("zstd-layer", {
            region,
            layerName: "zstd",
            code: new pulumi.asset.FileArchive(path.join(__dirname, "./zstd-layer")),
        }, { parent: this });
        ...
        const lambda = new aws.lambda.CallbackFunction("cwasm-compiler-lambda", {
            region,
            ...
            callback: async (event) => {
                const { S3Client, GetObjectCommand, DeleteObjectCommand, PutObjectCommand, } = await Promise.resolve().then(() => require("@aws-sdk/client-s3"));
                const { execSync } = await Promise.resolve().then(() => require("node:child_process"));
                const { readFileSync, createWriteStream } = await Promise.resolve().then(() => require("node:fs"));
                const { rm } = await Promise.resolve().then(() => require("node:fs/promises"));
                ...
                await pipeline(response.Body.transformToWebStream(), createWriteStream(wasmPath));
                ...
                inlinePolicies: [
                    {
                        ...
                        policy: pulumi
                            ...
                            .apply(([wasmBucketArn, cWasmBucketArn]) => JSON.stringify({
                            Version: "2012-10-17",
                            Statement: [
                                {
                                    Effect: "Allow",
...
exports.AwsLambdaCwasmCompiler = AwsLambdaCwasmCompiler;
//# sourceMappingURL=AwsLambdaCwasmCompiler.js.map