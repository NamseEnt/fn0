...
class CwasmS3Pusher extends pulumi.ComponentResource {
    constructor(name, args, opts) {
        super("pkg:index:cwasm-s3-pusher", name, args, opts);
        const { awsS3Region, zoneId, targetBuckets, cloudflareApiToken } = args;
        const cfProvider = new cloudflare.Provider("cwasm-cf-provider", { apiToken: cloudflareApiToken }, { parent: this });
        ...
        new aws.s3.BucketNotification("cwasm-notification", {
            region: awsS3Region,
            bucket: cwasmZstBucket.bucket,
            queues: [
                {
        ...
        const originAccessControl = new aws.cloudfront.OriginAccessControl("cwasm-oac", {
            name: "cwasm-zst-oac",
            ...
            inlinePolicies: [
                {
                    ...
                    policy: cwasmZstBucketArn.apply((bucketArn) => JSON.stringify({
                        Version: "2012-10-17",
                        Statement: [
                            {
                                Effect: "Allow",
        ...
        const lambda = new aws.lambda.CallbackFunction("s3-pusher-lambda", {
            region: awsS3Region,
            timeout: 60,
            memorySize: 128,
            environment: {
            ...
            callback: async (event) => {
                const { getSignedUrl } = await Promise.resolve().then(() => require("@aws-sdk/cloudfront-signer"));
                const { getSignedUrl: getS3SignedUrl } = await Promise.resolve().then(() => require("@aws-sdk/s3-request-presigner"));
                const { S3Client, DeleteObjectCommand, PutObjectCommand } = await Promise.resolve().then(() => require("@aws-sdk/client-s3"));
                ...
                const targetUrls = await Promise.all(targetBuckets.map(async (target) => {
...
exports.CwasmS3Pusher = CwasmS3Pusher;
//# sourceMappingURL=CwasmS3Pusher.js.map