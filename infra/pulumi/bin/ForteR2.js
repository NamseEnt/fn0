...
class ForteR2 extends pulumi.ComponentResource {
    constructor(name, args, opts) {
        ...
        new cloudflare.R2CustomDomain("custom-domain", {
            accountId,
        ...
        const r2PermissionIds = permissionGroups.apply((list) => {
            ...
            const read = groups.find((g) => g.name === "Workers R2 Storage Bucket Item Read");
            const write = groups.find((g) => g.name === "Workers R2 Storage Bucket Item Write");
            if (!read || !write) {
                throw new Error(`Could not find R2 permission groups; found names: ${groups
                    .map((g) => g.name)
                    .join(", ")}`);
            }
            return [{ id: read.id }, { id: write.id }];
        });
        const token = new cloudflare.AccountToken("r2-admin-token", {
            accountId,
            name: pulumi.interpolate `fn0-forte-r2-${bucket.name}`,
            policies: [
                {
                    effect: "allow",
                    resources: pulumi.all([accountId, bucket.name]).apply(([acct, name]) => ({
        ...
        this.secretAccessKey = pulumi.secret(token.value.apply((v) => crypto.createHash("sha256").update(v).digest("hex")));
    }
}
