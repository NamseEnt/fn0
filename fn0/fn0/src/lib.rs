...
pub use dibi_hijack::DibiHijack;
...
impl<C: BundleCache> ExecutionContext<C> {
    ...
    pub fn with_dibi_hijack(mut self, dibi_hijack: Arc<DibiHijack>) -> Self {
        self.dibi_hijack = Some(dibi_hijack);
        self
    }
...
impl<C: BundleCache> CodeExecutor<C> {
    ...
    fn wasm_instance_sender(
        ...
        let dibi_hijack = ctx.dibi_hijack.clone();
