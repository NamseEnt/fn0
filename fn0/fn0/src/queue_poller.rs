...
impl QueuePoller {
    ...
    where
        F: Fn(String, String) -> Pin<Box<dyn Future<Output = Result<()>>>>,
    {
        ...
        loop {
            ...
            match self.store.claim_task().await {
                Ok(Some(task)) => {
                    let result = (execute_fn)(task.task_name.clone(), task.payload.clone()).await;
                        ...
                        Err(e) => {
                            ...
                            if task.retry_count + 1 >= task.max_retries {
                                ...
                                if let Err(e) = self.store.retry_task(&task.id).await {
                                    tracing::error!("Failed to retry task {}: {e}", task.id);
                                }
                            }
                        }
                    }
