...
impl AccessorGuard {
    pub(crate) fn install(accessor: &Accessor<ClientState<SystemClock>>, service: &Service) -> Self {
            ...
            match handle_result {
                Ok(Ok(resp)) => {
                    let http_resp = accessor
                        ...
                        .map_err(|e| {
                            TrappableError::from(ErrorCode::InternalError(Some(format!(
                                "{e:?}"
                            ))))
                        })?;
