...
impl BundleCache for S3BundleCache {
    fn get(
        &self,
        subdomain: &str,
    ) -> impl std::future::Future<Output = Result<Arc<Bundle>, Error>> + '_ {
        ...
        async move {
            ...
            singleflight
                .work(&key, async move {
                    provider.get_impl(&key_for_task).await
                })
                .await
    ...
    fn invalidate(&self, subdomain: &str) -> impl std::future::Future<Output = ()> + '_ {
