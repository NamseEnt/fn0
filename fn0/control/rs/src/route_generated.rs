...
mod proxy {
    forte_sdk::wit_bindgen::generate!(
        ...
        path :
        "/Users/namse/fn0/fn0/control/rs/target/wasm32-wasip2/release/build/fn0-control-0b8e14669d171a77/out",
...
async fn handle_action(
    ...
    match action_name {
        ...
        "presign_quota" => {
            let input: crate::actions::presign_quota::Input =
                match forte_json::from_slice(body_bytes) {
                    Ok(v) => v,
                    Err(e) => {
                        return Ok(Response::builder()
            ...
            let output = crate::actions::presign_quota::handler(req).await;
            let json = forte_json::to_vec(&output);
            Ok(build_response_with_cookies(
                Response::builder()
                    .status(StatusCode::OK)
        ...
        "admin_run" => {
            let input: crate::actions::admin_run::Input = match forte_json::from_slice(body_bytes) {
                Ok(v) => v,
                Err(e) => {
                    return Ok(Response::builder()
                        .status(StatusCode::BAD_REQUEST)
            ...
            let output = crate::actions::admin_run::handler(req).await;
            let json = forte_json::to_vec(&output);
            Ok(build_response_with_cookies(
                Response::builder()
                    .status(StatusCode::OK)
        ...
        "usage_metering" => {
            let input: crate::actions::usage_metering::Input =
                match forte_json::from_slice(body_bytes) {
                    Ok(v) => v,
                    Err(e) => {
                        return Ok(Response::builder()
            ...
            let output = crate::actions::usage_metering::handler(req).await;
            let json = forte_json::to_vec(&output);
            Ok(build_response_with_cookies(
                Response::builder()
                    .status(StatusCode::OK)
